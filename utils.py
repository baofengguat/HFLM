...
# from utils_Package.layers import AmpNorm
from model import *
...
def load_n_center_lung_data(datadir):#目的是加载和处理肺部图像数据，并将结果存储在一个名为center_set的字典中。
    transform = transforms.Compose([transforms.ToTensor()])#将图像数据转换为张量格式
    center_set={}

    for center_name in os.listdir(datadir):
        sub_set = {}
        if center_name == ".DS_Store" or center_name == "江门医院": #其他本地模型不使用江门医院数据
            continue
    ...
    return center_set #函数返回center_set字典，其中包含了所有中心的训练和测试数据
def load_cifar10_data(datadir):#用于指定CIFAR-10数据集的下载和存储位置
...
def load_server_data(datadir):
    ...
    return (X_train, y_train, X_test, y_test)
def record_net_multicenter_data_stats(y_train_set, net_dataidx_map, logdir):#目的是计算多中心数据集的类别分布，并记录每个网络的类别计数和统计信息
    net_cls_counts = {}
...
def record_net_data_stats(y_train, net_dataidx_map, logdir):#存疑
    net_cls_counts = {}
    # print(net_dataidx_map.items())
    for net_i, dataidx in net_dataidx_map.items():
        unq, unq_cnt = np.unique(y_train[dataidx], return_counts=True)
...
def get_trainable_parameters(net, device='cpu'):#目的是返回神经网络的可训练参数值向量，用来获取神经网络中可训练参数的值，以便进行优化或分析
...
def put_trainable_parameters(net, X):#目的是将给定的向量（X）中的参数值替换为神经网络中可训练参数的值。
